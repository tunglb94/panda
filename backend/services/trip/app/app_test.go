...
func seedDeliveryTrip(t *testing.T, tripRepo *stubRepo, deliveryRepo *memory.DeliveryRepository, tripID, deliveryID string, deliveryStatus entity.DeliveryStatus) {
	...
	trip := entity.ReconstituteTrip(tripID, "rider1", "driver1", entity.StatusDriverArrived, "pickup", "dropoff", "", 0, "", "", now, now, entity.CompleteFinancials{}, nil, nil, 0, entity.TripSummary{})
...
func makeTrip(repo *stubRepo, tripID, riderID string, status entity.TripStatus) *entity.Trip {
	now := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	trip := entity.ReconstituteTrip(tripID, riderID, "", status, "pickup", "dropoff", "", 0, "", "", now, now, entity.CompleteFinancials{}, nil, nil, 0, entity.TripSummary{})
