...
**Still a near-total gap for actual driver functionality beyond the Home UI** — no auth, no KYC, no real availability/location API, no dispatch offer handling, no real navigation, no real earnings data. D2 and D4–D7 (Driver App Roadmap) remain not started; D3 (Go online/offline) now has its UI built (this phase) but not its real wiring.
...
- Flutter (Driver): `apps/driver/test/widget_test.dart` holds 8 smoke tests (3 from D-01: tab shell + all 5 destinations render, tab switching works, Developer page opens with all 4 diagnostic fields; 5 new from D-02: Home dashboard loads summary/stats, the toggle walks through all 4 availability states with the status card updating in sync, Empty state (no vehicle) and Error state via the dev preview menu, Quick Action tap shows its placeholder message).
...
| Offer handling, navigation, trip lifecycle, earnings (D4–D7) | Receive/accept dispatch offers, map navigation, start/complete trip, real earnings data | See Section 6 | Not started |
...
| Driver app automated tests | 8 smoke tests (`apps/driver/test/widget_test.dart`, D-01+D-02): tab shell/switching/Developer page (D-01); Home dashboard load, all 4 availability states, Empty/Error dev states, Quick Action placeholder (D-02) |
...
| D4. Offer handling | Receive dispatch offer, accept/reject with timeout | Dispatch API (exists) | Medium |
| D5. Navigation to pickup/dropoff | Map + directions during active trip | Trip API (exists), a maps/directions provider | Medium-High |
